...
var require_buffer_list = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var inspect = require___vite_browser_external().inspect;
...
var require__stream_readable = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var debugUtil = require___vite_browser_external();
	var debug;
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-parser.js
var require_rtf_parser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Transform = require_readable_browser().Transform;
	var RTFParser = class extends Transform {
		constructor() {
		...
		parseControlSymbol(char) {
			...
			else if (char === "_") this.text += "‑";
			else if (char === "*") {
				if (typeof disableIgnorable !== "undefined" && !disableIgnorable) this.emitIgnorable();
				this.parserState = this.parseText;
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-group.js
var require_rtf_group = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var RTFGroup = class {
		constructor(parent) {
			this.parent = parent;
			this.content = [];
		...
		addContent(node) {
			if (this.parent) node.controls = this.controls;
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-paragraph.js
var require_rtf_paragraph = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var RTFParagraph = class {
		constructor(opts) {
			if (!opts) opts = {};
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-document.js
var require_rtf_document = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var RTFGroup = require_rtf_group();
	var RTFParagraph = require_rtf_paragraph();
	var RTFDocument = class extends RTFGroup {
		constructor() {
			super();
			this.charset = "ASCII";
...
var buffer, base64Js, lookup, revLookup, Arr, code, i, len, ieee754, Buffer$1;
...
var require_assert = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var inspect = require_util().inspect;
	var _require$types = require_util().types, isPromise = _require$types.isPromise, isRegExp = _require$types.isRegExp;
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-span.js
var require_rtf_span = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var RTFSpan = class {
		constructor(opts) {
			if (!opts) opts = {};
...
//#endregion
//#region resources/js/rtf-parser-sdarm/rtf-interpreter.js
var require_rtf_interpreter = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	init_dist();
	init_dist$1();
	...
	var RTFInterpreter = class extends Writable {
		...
		_write(cmd, encoding, done) {
			...
			else process$1.emit("error", `Unknown RTF command ${cmd.type}, tried ${method}`);
			done();
		}
		finisher() {
		...
		flushHexStore() {
			if (this.hexStore.length > 0) {
				if (this.group.controls["uc"] == void 0 || this.group.controls["uc"] != 2 || this.group.content[this.group.content.length - 1].controls["u"] == void 0) {
					let hexstr = this.hexStore.map((cmd) => cmd.value).join("");
					this.group.addContent(new RTFSpan({ value: iconv.decode(Buffer$1.from(hexstr, "hex"), this.group.get("charset")) }));
		...
		cmd$groupEnd() {
			...
			if (endingGroup instanceof FontTable) doc.fonts = endingGroup.table;
				...
				process$1.emit("debug", "GROUP END", endingGroup.type, endingGroup.get("ignorable"));
			}
		}
		cmd$text(cmd) {
			this.flushHexStore();
		...
		cmd$controlWord(cmd) {
			...
			this.group.controls[cmd.value] = cmd.param.toString();
			const method = "ctrl$" + cmd.value.replace(/-(.)/g, (_, char) => char.toUpperCase());
			if (this[method]) this[method](cmd.param);
			else if (!this.group.get("ignorable")) process$1.emit("debug", method, cmd.param);
		}
		cmd$hexchar(cmd) {
			this.hexStore.push(cmd);
		}
		cmd$error(cmd) {
			this.emit("error", /* @__PURE__ */ new Error("Error: " + cmd.value + (cmd.row && cmd.col ? " at line " + cmd.row + ":" + cmd.col : "") + "."));
		}
		ctrl$endash(cmd) {
		...
		ctrl$u(num) {
			var charBuf = Buffer$1.alloc ? Buffer$1.alloc(2) : new Buffer$1(2);
			charBuf.writeInt16LE(num, 0, true);
		...
		ctrl$ansicpg(codepage) {
			if (availableCP.indexOf(codepage) === -1) this.emit("error", /* @__PURE__ */ new Error("Codepage " + codepage + " is not available."));
		...
		ctrl$f(num) {
			if (this.group instanceof FontTable) this.group.currentFont = this.group.table[num] = new Font();
				...
				fontCharset = fontCharset && fontCharset !== "ASCII" ? fontCharset : this.group.charset;
		...
		ctrl$fcharset(code) {
			if (this.group instanceof FontTable || this.group.parent instanceof FontTable) {
				...
				else if (code === 0) charset = this.doc.charset;
				else charset = codeToCP[code];
				if (charset == null) charset = codeToCP[0];
	...
	var FontTable = class extends RTFGroup {
		constructor(parent) {
			super(parent);
			this.table = [];
			this.currentFont = {
	...
	var ColorTable = class extends RTFGroup {
		constructor(parent) {
			super(parent);
			this.table = [];
			this.red = 0;
...
//#endregion
//#region resources/js/article_converter.js
var import_rtf_parser_sdarm = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	var RTFParser = require_rtf_parser();
	var RTFDocument = require_rtf_document();
	var RTFInterpreter = require_rtf_interpreter();
...
$(function() {
	function idbStorage(documentID, tableName, dbName, initFunction) {
		...
		this.dataStack = [];
		this.iDB = null;
		var self = this, indexedDB = window.indexedDB || window.webkitIndexedDB || window.mozIndexedDB || window.OIndexedDB || window.msIndexedDB;
		...
		openRequest.onupgradeneeded = function(e) {
			...
			e.target.result.createObjectStore(tableName);
		};
		openRequest.onsuccess = function(e) {
			...
			var request = self.iDB.transaction([tableName], "readonly").objectStore(tableName).get(documentID);
			request.onsuccess = function(e) {
				if (e.target.result != void 0) {
					var result = e.target.result;
					self.dataStack = JSON.parse(result);
					if (!self.canUndo) $("#command-undo").addClass("disabled");
					else $("#command-undo").removeClass("disabled");
				}
		...
		this.put = function(key, data) {
			...
			var request = transaction.objectStore(tableName).put(data, key);
			request.onerror = function(e) {
		...
		this.get = function(key, fn) {
			var request = self.iDB.transaction([tableName], "readonly").objectStore(tableName).get(key);
			request.onsuccess = function(e) {
				var result = e.target.result;
				fn(result);
			};
		...
		this.canUndo = function() {
			if (this.dataStack.length > 1) return true;
			else return false;
		};
		this.putStore = function(data, factor) {
			var elementId = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : "0";
		...
		this.getAll = function() {
			if (this.dataStack.length > 0) return this.dataStack;
			else return null;
		};
	}
	var dataStore = null;
	$("#file-source").change(function(e) {
		...
		if (file.name.endsWith(".rtf") || file.name.endsWith(".txt")) {
			...
			reader.readAsText(file, "UTF-8");
		} else alert("ERROR:Unknown file type!");
	});
	$("#book-name, #lang, #year, #issue").change(function() {
		if ($("#book-name").val() != "" && $("#lang").val() != "" && $("#year").val() != "" && $("#issue").val() != "") {
			if ($("#book-name").val() == "rmrh") {
				...
				$("#volume").val((parseInt($("#year").val()) - 2020 + 61).toString());
			} else if ($("#book-name").val() == "ym") {
				$("#volume").val((parseInt($("#year").val()) - 2020 + 39).toString());
			...
			dataStore = new idbStorage("".concat($("#book-name").val(), "-", $("#lang").val(), "-", $("#year").val(), "-", $("#issue").val()), "RtfToHtml", "ArticleConverter", function() {
				content = dataStore.getCurrent();
				w = $("#panel-editor-content").width();
				if (content) {
					$("#panel-editor-content").html(content);
					if ($(".selected").length > 0) $("#panel-editor-content").scrollTop($(".selected").position().top);
				} else {
	...
	function parseRtf(content) {
		...
		} else content = content.replace(/((\\uld)([\\ ]))|((\\uldash)([\\ ]))|((\\uldashd)([\\ ]))|((\\uldashdd)([\\ ]))|((\\ulnone)([\\ ]))|((\\ulth)([\\ ]))|((\\ulw)([\\ ]))|((\\ulwave)([\\ ]))/g, "\\ul$3$6$9$12$15$18$21$24");
		import_rtf_parser_sdarm.default.string(content, function(err, doc) {
			if (err) alert(err.toString());
			else {
				var element = document.getElementById("panel-editor-content");
				element.innerHTML = "";
				element.insertAdjacentHTML("beforeend", toHtml(doc));
				if (dataStore) dataStore.putStore($("#panel-editor-content").html(), 0);
				else dataStore = new idbStorage("".concat($("#book-name").val(), "-", $("#lang").val(), "-", $("#year").val(), "-", $("#issue").val()), "RtfToHtml", "ArticleConverter", function() {
					dataStore.putStore($("#panel-editor-content").html(), 0);
				});
			}
		});
	...
	function toHtml(doc) {
		...
		doc.content.forEach(function myFunction(item, indexParag) {
			if (item.value != void 0) {
				...
				console.log(_str);
			} else if (item.content.length == 0) return;
			else {
				let paragInnerHtml = "";
				let strText = "";
				if (item.content.length == 1) strText = paragInnerHtml = item.content[0].value.trim();
				else {
				...
				if (strText.match($("#pattern-article").val())) {
					if (mode == "sect") strArticle += "<div class=\"sect1\">" + strSection + "</div>";
					else if (mode == "bibliography") strArticle += "<div class=\"bibliography\">" + strSection + "</div>";
					if (mode != "") strHtml += "<div class=\"article\">" + strArticle + "</div>";
					mode = "article";
					titleCount = 0;
					isFirstParag = true;
					if (strText.indexOf("@") == 0) {
						...
						titleCount++;
					} else strArticle = "<p class=\"keyword\">" + strText + "</p>";
				} else if (mode == "bibliography") if (strText.match($("#pattern-biblioentry").val())) {
					...
					if (strText.match($("#pattern-article").val())) {
						...
						strArticle += "<div class=\"abstract\"><p class=\"\"></p></div>";
					} else if (strText.match($("#pattern-title").val())) {
						if (strText.indexOf("@") == 0) strText = strText.replace("@", "");
						strArticle = "<p class=\"title\">" + strText + "</p>";
						strArticle += "<div class=\"abstract\"><p class=\"\"></p></div>";
						titleCount++;
					} else {
				...
				else if (strText.match($("#pattern-bibliography").val())) {
					if (mode == "sect") strArticle += "<div class=\"sect1\">" + strSection + "</div>";
					strSection = "<p class=\"title\">" + strText + "</p>";
					mode = "bibliography";
					citationNo = 1;
				} else if (strText.length < 50 && strText.match($("#pattern-auther").val())) {
					if (mode == "sect") {
						strHtml += "<div class=\"article\">" + strArticle + "</div>";
						mode = "article";
					...
					if (strText.indexOf("#") == 0) strText = strText.slice(1);
					if (titleCount == 0) {
						strArticle += "<p class=\"title\">" + strText + "</p>";
						strArticle += "<div class=\"abstract\"><p class=\"\"></p></div>";
						isFirstParag = true;
					...
					if (mode == "article") strArticle += strParag;
					else if (mode == "sect") strSection += strParag;
					else strHtml += strParag;
				}
			}
		});
		if (mode == "sect") strArticle += "<div class=\"sect1\">" + strSection + "</div>";
	...
	function doSpan(item, parentParag) {
		var classAttr = "";
		if (item.controls !== void 0) {
			if (item.controls["cs"] != void 0 && item.controls["cs"] == $("#citation-style").val()) classAttr += " citation";
			else if (item.controls["super"] != void 0 && item.value.match(/^\d+$/)) classAttr += " citation";
			if (item.controls["b"] != void 0) if (item.controls["b"] == "0") item.style.bold = false;
			else classAttr += " bold";
			if (item.controls["i"] != void 0) if (item.controls["i"] == "0") item.style.italic = false;
			else classAttr += " italic";
			if (item.controls["ul"] != void 0) classAttr += " underline";
		}
	...
	function styleToString(style) {
		...
		for (var property in style) if (style.hasOwnProperty(property)) {
			if (property == "bold" && style[property]) css += " bold";
			else if (property == "italic" && style[property]) css += " italic";
	...
	$("#panel-editor-content").on("keyup", function(event) {
		...
		} else if (code === 27) {} else if (event.ctrlKey) {
			if (code === 90 || code === 122) {} else if (event.shiftKey && event.altKey && (code === 82 || code === 114)) {} else if (event.shiftKey && event.altKey && (code === 68 || code === 100)) {}
		}
	});
		...
		if (e.target.compareDocumentPosition(document.getElementById("panel-editor-content")) & Node.DOCUMENT_POSITION_CONTAINS) {
			...
			if (selectionRange.collapsed) {
				...
				if (element.attributes.length > 1) {
					tags += "[";
					for (let idx = 0; idx < element.attributes.length; idx++) if (element.attributes[idx].name != "class") tags += "@" + element.attributes[idx].name + "=" + element.attributes[idx].value;
					tags += "]";
				}
				while (element.id != "panel-editor-content") {
					tags = "<div style='display:inline; margin:0; padding:0' class='" + element.className + " selected-element-indicator'>&gt;&nbsp;" + element.tagName + "." + element.className + tags;
	...
	$(".menu").click(function(e) {
		...
		if (this.className.includes("block-attr")) {
			...
			while (curElement) {
				...
				if (curElement.classList.contains("book")) break;
			...
			commonParentIndex++;
			if (commonParentElement == startParaElement) commonParentElement = startParaElement.parentElement;
			function addAttr(attr, startChildElement, endChildElement) {
				...
				if ((attr.includes("sect") || attr.includes("bibliography")) && el.textContent.match($("#pattern-title").val())) {
				...
				do {
					...
					if ([
						...
						"annotation",
						"epigraph"
					].includes(attr)) insertElement.innerHTML = insertElement.textContent;
					...
					if (insertElement == endChildElement) break;
				} while (el && !el.classList.contains(attr));
				if (el) {
					parentElement.insertBefore(wrappingElement, el);
					if (insertElement == endChildElement) el = null;
				} else parentElement.appendChild(wrappingElement);
				return el;
			}
			function removeAttr(element) {
				let parent = element.parentElement;
				while (element.firstElementChild) parent.insertBefore(element.firstElementChild, element);
				parent.removeChild(element);
			}
			if (commonParentElement.classList.contains(attr)) removeAttr(commonParentElement);
			else {
				...
				if (attrOrder[attr] <= attrOrder[commonParentElement.className] || attrOrder[attr] > attrOrder[curElement.className]) {
				...
				do
					if (curElement.classList.contains(attr)) {
						...
						if (curElement == endElement) curElement = null;
						else curElement = curElement.nextElementSibling;
						removeAttr(child);
					} else curElement = addAttr(attr, curElement, endElement);
				while (curElement);
			}
			...
			do {
				...
				if (attr == "plain") startParaElement.className = "";
				else startParaElement.className = attr;
				startParaElement = nextElementSibling;
			} while (startParaElement && startParaElement != nextGroupElement);
			window.getSelection().empty();
			selectionRange = null;
			$("#panel-editor-content *").removeClass("selected-attr-block");
			$(".menu-attr").addClass("disabled");
			$("#selection-indicator").html("");
		} else if (this.className.includes("parag-attr")) {} else if (this.className.includes("span-attr")) {
			if (selectionRange.collapsed) return;
			function applyAttr(containner) {
				do {
					if (containner instanceof Element) containner = containner.firstChild;
					...
					if (containner.tagName != "SPAN") {
						...
						if (containner.classList.contains(attr)) if (containner.classList.length == 1) newAttrNode = document.createTextNode(newAttrText);
						else {
							newAttrNode = document.createElement("span");
					...
					if (newAttrNode instanceof Element) newAttrNode = newAttrNode.firstChild;
					if (theContainner == selectionRange.startContainer) selectionRange.setStart(newAttrNode, 0);
					const oldEndSelectionContainer = selectionRange.endContainer;
					if (theContainner == selectionRange.endContainer) selectionRange.setEnd(newAttrNode, newAttrNode.textContent.length);
					const oldContainer = containner;
					if (theContainner != oldEndSelectionContainer) containner = containner.nextSibling;
					else containner = null;
					oldContainer.remove();
				} while (containner);
			}
			let stop = false;
			...
			function traverse(element) {
				if (stop || !element || element.classList.contains("book")) return;
				if (element.tagName != "P") for (const childElement of element.children) {
					traverse(childElement);
					if (stop) return;
				}
				else {
					if (applying || element == startParaElement) {
						if (!element.classList.contains("keyword") && !element.classList.contains("title") && !element.classList.contains("subtitle") && !element.classList.contains("part-no") && !element.classList.contains("author") && !element.classList.contains("biblioentry")) applyAttr(element == startParaElement ? selectionRange.startContainer : element.firstChild);
						applying = true;
					}
					if (element == endParaElement) stop = true;
					element = element.nextElementSibling;
					traverse(element);
				}
			}
			let bookChild = startParaElement;
			while (!bookChild.parentElement.className.includes("book")) bookChild = bookChild.parentElement;
			while (bookChild) {
				traverse(bookChild);
				if (stop) break;
	...
	function convertParagChild(paraElement) {
		...
		try {
			if (paraElement.className.length > 0 && paraElement.className.includes("subtitle")) {
				...
				if (volume) $(infoElement).append("<volumenum>" + volume[0] + "</volumenum>");
				else $(infoElement).append("<volumenum>" + paraElement.innerText + "</volumenum>");
				...
				if (tmp.length > 1) {
					names = tmp[0].trim();
					country = tmp[1].trim();
				} else names = tmp[0].trim();
				var nameArray = names.trim().replace("By ", "").replace("by ", "").replace("  ", " ").split(" ");
				if (nameArray.length == 3) xmlString = "<personname><firstname>" + nameArray[0] + "</firstname><othername>" + nameArray[1] + "</othername><surname>" + nameArray[2] + "</surname></personname>";
				else if (nameArray.length == 2) xmlString = "<personname><firstname>" + nameArray[0] + "</firstname><surname>" + nameArray[1] + "</surname></personname>";
				else xmlString = "<personname><othername>" + nameArray.join(" ") + "</othername></personname>";
				if (country.length > 0) xmlString += "<address><country>" + country + "</country></address>";
				...
				for (spanIndex = 0; spanIndex < newParag.childNodes.length; spanIndex++) {
					...
					spanRoles = spanElement.className == void 0 ? "" : spanElement.className.trim();
					var newSpan = null;
					if (spanElement.className.includes("olink")) {
						newSpan = document.createElement("olink");
					...
					if (newSpan) newParag.replaceChild(newSpan, spanElement);
				}
			}
		} catch (error) {
	...
	function convertSectionChild(sectionElement) {
		...
		section.innerHTML = sectionElement.innerHTML;
		...
		for (var _index = 0; _index < section.children.length; _index++) {
			...
			var newElement = null;
			if (element.className?.includes("sect") || element.className?.includes("epigraph") || element.className?.includes("bibliography")) newElement = convertSectionChild(element);
			else if (element.className?.includes("abstract") || element.className?.includes("annotation")) blockInfoElement.append(convertSectionChild(element));
			else newElement = convertParagChild(element);
			if (newElement) section.replaceChild(newElement, element);
			else {
	...
	function exportXML() {
		...
		xmlRoot.setAttribute("status", (/* @__PURE__ */ new Date()).toISOString());
		xmlRoot.setAttribute("xml:id", $("#book-name").val() + "-" + $("#lang").val() + "-" + $("#year").val() + "-" + $("#issue").val());
		...
		for (index = 0; index < articleList.length; ++index) {
			...
			for (let _index2 = 0; _index2 < newArticle.children.length; _index2++) try {
				var element = newArticle.children[_index2];
				var newElement = null;
				if (element.className.includes("sect") || element.className.includes("epigraph") || element.className.includes("bibliography")) newElement = convertSectionChild(element);
				else if (element.className.includes("abstract") || element.className.includes("annotation")) infoElement.append(convertSectionChild(element));
				else newElement = convertParagChild(element);
				if (newElement) {
					if (newArticle != element.parentElement) lert("Error: Report to Sean, Please!!!");
			...
			if (articleRole.length) newArticle.setAttribute("role", articleRole);
			...
			if (titleCollection.length == 0) titleCollection = newArticle.getElementsByTagName("TITLE");
			if (titleCollection.length > 0) newArticle.insertBefore(infoElement, titleCollection[0].nextSibling);
			infoElement = null;
			articleRole = "";
		...
		if (document.getElementById("auto-upload").checked) $("#button-upload-xml").click();
	}
	var prettifyXml = function prettifyXml(sourceXml) {
		...
		return new XMLSerializer().serializeToString(resultDoc).replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n", "");
	};
	...
	$("#button-do-validation").click(function() {
		...
		for (let idx = 1; idx <= articleCount; idx++) {
			...
			for (let jdx = 1; jdx <= articleCount; jdx++) if (jdx != idx) bookCopy.querySelector(`article[xml\\:id="article-${jdx}"]`).remove();
			...
			$.ajax({
				...
				if (data.code == -1) msg += "  FAILED\n";
				if (Array.isArray(data.message)) data.message.forEach((val) => {
					msg += val + "\n";
				});
	...
	$("#panel-editor-content").scroll(function() {
		...
		if (Math.abs(scrollPos - prevScrollPos) < 20) return;
		var rect = null;
		var book = $("#book-name").val();
		...
		if (book == "sbl" || book == "sblpab") {
			...
			$(this).children(".foreword, .fso, .lesson").each(function() {
				...
				if (rect.top > headerHeight) {
					if (prevElement.tagName == "FSO") $("#cur-pos-indicator").text("FSO " + fso.toString());
					else if (prevElement.tagName == "LESSON") $("#cur-pos-indicator").text("LESSON " + prevElement.getAttribute("no"));
					else $("#cur-pos-indicator").text(prevElement.tagName);
					return false;
				}
				prevElement = this;
			});
			if (rect.top <= headerHeight) $("#cur-pos-indicator").text("LESSON " + prevElement.getAttribute("no"));
	...
	$("#cur-pos-indicator").click(function() {
		...
		if (goto) goto.parentNode.removeChild(goto);
		var book = $("#book-name").val();
		...
		if (book == "sbl" || book == "sblpab") {
			...
			for (idx = 0; idx < articles.length; idx++) if (articles[idx].tagName == "LESSON") html += "<option value='" + $(articles[idx]).attr("no") + "'>LESSON " + $(articles[idx]).attr("no") + "</option>";
			else html += "<option value='" + $(articles[idx]).attr("date") + "'>FSO " + $(articles[idx]).attr("date") + "</option>";
		} else {
			articles = $("#panel-editor-content > .article");
			html += articles.length + "'>";
			for (var number = 1; number <= articles.length; number++) html += "<option value='" + (number - 1).toString() + "'>Article " + number.toString() + "</option>";
		}
		html += "</select></div>";
		this.innerHTML += html;
	});
	$(document).on("change", "#select-goto", function() {
		...
		var book = $("#book-name").val();
		if (book == "sbl" || book == "sblpab") if (parseInt(value) < 15) $("#panel-editor-content").animate({ scrollTop: $("lesson[no='" + value + "']").offset().top + headerHeight + 100 }, 500);
		else $("#panel-editor-content").animate({ scrollTop: $("fso[date='" + value + "']").offset().top + headerHeight + 100 }, 500);
		else $("#panel-editor-content").animate({ scrollTop: $(".article:eq(" + value + ")").offset().top + scrollPos - headerHeight }, 500);
	});
	$(document).on("click", function(event) {
		var goto = document.getElementById("select-goto-wrapper");
		if (goto && !event.target.closest("#cur-pos-indicator")) goto.parentNode.removeChild(goto);
	});
	...
	dragElement(document.getElementById("panel-spliter-vertical"), document.getElementById("panel-editor-wrapper"), document.getElementById("panel-source-wrapper"), "V");
	dragElement(document.getElementById("panel-spliter-horizontal"), document.getElementById("panel-source"), document.getElementById("panel-logger"), "H");
